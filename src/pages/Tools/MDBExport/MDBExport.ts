import { MDBToCB } from "../../../tools/MDBToCBExport";
import {
  CBTools,
  Type as CBToolsType,
...
import { dataMdbMigrateWebview } from "../../../webViews/tools/dataMdbMigrate.webview";
...
export const mdbExport = async (context: vscode.ExtensionContext) => {
  ...
  const dataMigratetWebviewDetails = Memory.state.get<IDataMigrateWebviewState>(
  ...
  async function connectToMongoDB(connectionString: string): Promise<string[]> {
    try {
      ...
      let error_message = "Unable to Connect to MongoDB, Please check Connection URI and try again";
      console.error("Error while connecting to MongoDB:", error);
      currentPanel.webview.postMessage({
        command: "vscode-couchbase.tools.mdbExport.formValidationError",
        error: error_message,
      });
  ...
  async function getCollectionsList(connectionString: string, databaseName: string): Promise<string[]> {
    try {
      const client: MongoClient = await MongoClient.connect(connectionString);
  ...
  try {
    currentPanel.webview.html = await dataMdbMigrateWebview(bucketNameArr);
    currentPanel.webview.onDidReceiveMessage(async (message) => {
      switch (message.command) {
        // ADD cases here :)
        case "vscode-couchbase.tools.mdbExport.runExport":
          ...
          break;
        case "vscode-couchbase.tools.mdbExport.getCollections":
          const collection = await getCollectionsList(message.connectionString, message.databases);
          currentPanel.webview.postMessage({
            command: "vscode-couchbase.tools.mdbExport.collectionInfo",
            collection: collection
          });

          break;

        case "vscode-couchbase.tools.mdbExport.getCbScopes":
          ...
          currentPanel.webview.postMessage({
            command: "vscode-couchbase.tools.mdbExport.scopesInfo",
            scopes: scopes,
          });
          break;

        case "vscode-couchbase.tools.mdbExport.getDatabases":
          const databases = await connectToMongoDB(message.connectionString);
          currentPanel.webview.postMessage({
            command: "vscode-couchbase.tools.mdbExport.databaseInfo",
            databases: databases,
          });
