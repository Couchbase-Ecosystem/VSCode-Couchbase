...
suite("CBSShapeCodeCompletion Test Suite", () => {
    ...
    test("completion for geometry collection", async () => {
        ...
        assert.ok(
            completionResults.includes("geometries"),
            "Expected completion 'geometries' not found",
        );
    });
    ...
    test("completion for polygon", async () => {
        ...
        assert.strictEqual(completionResults.length, 1);
        assert.ok(
            completionResults.includes("coordinates"),
            "Expected completion 'coordinates' not found",
        );
    });
});
