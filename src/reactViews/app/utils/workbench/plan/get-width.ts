...
export const getWidth = (
  node: {
    data: Partial<HierarchyNode<ExecutionPlanNode>['data']>;
  },
  compact: boolean
) => {
  if (compact) {
  ...
  if (node.data && node.data.name && node.data.name.length > maxWidth) maxWidth = node.data.name.length;
  if (node.data && node.data.details)
    for (let i = 0; i < node.data.details.length; i++)
      if (node.data.details[i] && node.data.details[i].length > maxWidth) maxWidth = node.data.details[i].length;
