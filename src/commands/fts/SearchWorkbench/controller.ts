 ...
 */
    public async openSearchJsonTextDocument(memFs: MemFS): Promise<Thenable<vscode.TextDocument>> {
        const uri = vscode.Uri.parse(`couchbase:/search-workbench-${this.untitledCount}.cbs.json`);
        this.untitledCount++;
    ...
    public newQuery(memFs: MemFS): Promise<boolean> {
        return new Promise<boolean>((resolve, reject) => {
            try {
                ...
                this.openSearchJsonTextDocument(memFs);
            } catch (error) {
                reject(error);
            }
        });
