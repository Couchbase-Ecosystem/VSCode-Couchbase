...
const sourceDirs = ['components/visual-explain-plan'];
...
const analyzeImports = (filePath, destinationPath) => {
  ...
  const visitNode = (node) => {
    if (ts.isImportDeclaration(node)) {
      ...
      const specialDirectories = new Set(["utils", "constants", "types", "error", "sync"]);

      if (doesFileExist(sourceComponentPath)) {
        const directoryName = path.dirname(importPath);
      
        if (specialDirectories.has(directoryName)) {
